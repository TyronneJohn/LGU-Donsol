...
export default function BacDashboard() {
  ...
  async function loadProjects() {
    ...
    const { data: projectRows, error: projectsError } = await supabase
      .from('projects')
      .select('id, project_code, title, status, estimated_cost, approved_budget, pow_amount, offices(name)')
  ...
  const totalAbc = projects.reduce(
    (sum, p) => sum + Number(p.procurement?.abc_amount ?? p.pow_amount ?? p.approved_budget ?? 0),
    0,
  )
                    ...
                    {visibleProjects.map((project) => (
                      <tr key={project.id}>
                        ...
                        <td className="px-4 py-2.5 text-slate-600">
                          {formatCurrency(
                            project.procurement?.abc_amount ?? project.pow_amount ?? project.approved_budget,
                          )}
                        </td>
